...
# This script has been tested and verified on Ubuntu 24.04 with these kernel versions on the
# latest patch version from https://cdn.kernel.org/pub/linux/kernel.
TESTED_KERNS=(
  3.19.8
  4.1.52
  4.7.10
...
# The ubuntu-latest runner uses GCC 13. Newer GCC versions have stricter static-analysis checks,
# which may flag issues in older kernels. All warnings are treated as errors and will stop
# compilation, however since these are just warnings and not outright errors, it is okay to proceed
# with compilation.
#
# We can either compile them using older GCC versions, or use most recent GCC and apply
# flags/patches. The logic below is the latter method.
case "$MAJOR.$MINOR.$PATCH" in
  3.19.8)
    # Newer GCC versions give the error: `include/linux/compiler-gcc.h: fatal error: linux/compiler-gcc13.h: No such file or directory`.
    # A hacky workaround is to copy and rename `include/linux/compiler-gcc*.h` to
    # `include/linux/compiler-gcc13.h`. Not sure why this works, but should be okay since it builds
    # successfully.
  ...
  3.19.8 | 4.7.10)
    ...
    # isn't a direct patch file, so we extract it from: https://lists.ubuntu.com/archives/kernel-team/2016-May/077178.html
  ...
  4.15.18)
    ...
    # `.fixdep.o.cmd:1: *** missing separator.`. Both patches below will need to be applied.
    # https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=9feeb638cde083c737e295c0547f1b4f28e99583
    # https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=9564a8cf422d7b58f6e857e3546d346fa970191e
    ...
    # definition of '__force_order'`. There is no patch for the `pagetable.c` file specifically, so
    # we instead pass the `-fcommon` flag.
    # https://lkml.iu.edu/2003.1/03102.html
    CC_FLAGS+=" -fcommon"
  ...
  4.13.16 | 4.15.18 | 4.17.19 | 4.18.20 | 4.20.17)
    ...
    wget https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/patch/?id=ff1bf4c0714e7936330bb316090a75eaa35061e7 \
      -O pf_max.patch
    # The patch is not exact, so we will need to adjust for abscence of
    # `#include <linux/kconfig.h>` in `scripts/selinux/mdp/mdp.c` with fuzz.
    patch -F3 -p1 < pf_max.patch
    ;;&
  4.18.20 | 4.20.17 | 5.2.21)
    # GCC 10+ `-fcf-protection=branch` flag conflicts with `-mindirect-branch`, which gives the
    # error: `'-mindirect-branch' and '-fcf-protection' are not compatible`
    # https://lore.kernel.org/lkml/20210208162543.GH17908@zn.tnic
    #
    # wget https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/patch/?id=20bf2b378729c4a0366a53e2018a0b70ace94bcd \
    #   -O disable_cet.patch
    # patch -p1 < disable_cet.patch

    # Applying the patch fails for some reason, so instead pass it in build.
  ...
  4.17.19 | 4.18.20 | 4.20.17 | 5.2.21 | 5.3.18)
    ...
    # definition of '__force_order'`. We can either apply `-fcommon` or apply the following patch.
...
# Build the compressed kernel image with default configuration.
make defconfig
make -j"$(nproc)" \
  ...
  bzImage
cp arch/x86/boot/bzImage ../vmlinuz-"${KERN}"
